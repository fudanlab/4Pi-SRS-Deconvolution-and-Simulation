...
folder = fileparts('C:\Users\jonik\OneDrive\Desktop\FuLab\4Pi-SRS\Supplemental80nm\supplemental1_bead_deconv\80nm\');
addpath(genpath(folder));

% 2) Load your data
rawFile      = fullfile(folder,'beforedeconv.tif');
...
% --- run change psf between 
% psf = E. Coli and beads
% psfzoom4 = A549
psf_deconv = psf;
f = deconvlucy(f, psf_deconv, 5);

% --- params (add stop rules; keep your weights) ---
params.iter        = 50;        % hard cap (e.g., 30–50)
params.mu          = 20;
params.lambda_tv   = 0.003;
params.lambda_l0   = 0.05;
...
% psf = E. Coli and beads
% psfzoom4 = A549
[f, diag] = deconv_residual(f, psf_deconv, params);


toc
%%
% --- quick convergence plots ---
figure('Color','w','Position',[80 80 500 320]);

subplot(1,2,1); 
semilogy(diag.iter, diag.data_rel,'-o','LineWidth',2); 
grid on; box on;
xlabel('iterations','FontSize',18); 
ylabel('||H*o-g||/||g||','FontSize',18); 
title('data residual','FontSize',18);
set(gca,'FontSize',16);   % makes tick labels larger

subplot(1,2,2); 
semilogy(diag.iter, diag.rel_change,'-o','LineWidth',2); hold on;
yline(params.stop.rel_change_tol,'--'); 
grid on; box on; 
xlabel('iterations','FontSize',18); 
ylabel('||Δo||/||o||','FontSize',18); 
title('relative change','FontSize',18);
set(gca,'FontSize',16);   % makes tick labels larger
...
writeStackToTiff(f , 'testdeconv.tif');
